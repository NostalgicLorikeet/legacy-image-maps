    ...
    private void run(ICommandSender sender, String[] args) {
        ...
        if (!world.isRemote) {
            THREAD_POOL.execute(() -> {
                ...
                if (now - ImageMapUtils.lastRequestTime > requiredRequestInterval) {
                    ImageMapUtils.lastRequestTime = Instant.now().toEpochMilli();
                    String url = "";
                    int scale = -1;
                    int width;
                    int height;
                    ...
                    imagemap.notifyWait(requiredRequestInterval - (now - ImageMapUtils.lastRequestTime));
                }
            });
        }
    }
